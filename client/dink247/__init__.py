...
class Client:
    def __init__(self, secret: Union[str, bytes, None]=SHARED_SECRET, service_url: str=SERVICE_URL) -> None:
        if secret is None:
    ...
    def create(self, url: str, base_url: Optional[str]=None, ttl: int=0) -> str:
        r = requests.post(
            self.service_url,
            self.sign(url, ttl),
